...
nameCollection = ["Log_Run_D_CG","Log_Run_D_COBYLA","Log_Run_D_L-BFGS-B","Log_Run_D_Nelder-Mead","Log_Run_D_Powell","Log_Run_D_SLSQP","Log_Run_D_TNC","Log_Run_D_trust-constr"]
xlist = []
ylist = []

for fileName in nameCollection:

    X, Y, Z = read_file("RunSequent\\"+fileName+".txt")
    Y = Y[:,1:]
    Z = Z[:,1:]
    Y *= -1000
    Z *= -1
    ...
    plt.text(3.5,0, TextVal, fontsize=14)
    plt.xlabel('Strain')
    plt.ylabel('Stress')
    plt.title(fileName+" : "+str(len(X))+' Step :F = '+str(F_value))
    plt.legend()
    # plt.show()
    plt.savefig('savePng/'+fileName+'.png')
    ylist.append(F_value)

draw_dot_plot(xlist,ylist,nameCollection)